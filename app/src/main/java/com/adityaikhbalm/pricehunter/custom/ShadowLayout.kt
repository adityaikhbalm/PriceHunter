...
class ShadowLayout(
    ...
    private var isShadowed: Boolean
        get() = mIsShadowed
        set(isShadowed) {
    ...
    @setparam:FloatRange(from = MIN_ANGLE.toDouble(), to = MAX_ANGLE.toDouble())
    private var shadowAngle: Float
        get() = mShadowAngle
        set(shadowAngle) {
            mShadowAngle = max(MIN_ANGLE, min(shadowAngle, MAX_ANGLE))
            resetShadow()
        }
    ...
    private var shadowRadius: Float
        get() = mShadowRadius
        set(shadowRadius) {
            mShadowRadius = max(MIN_RADIUS, shadowRadius)
    ...
    private fun resetShadow() {
        ...
        shadowDx = (mShadowDistance * cos(mShadowAngle / 180.0f * Math.PI)).toFloat()
        shadowDy = (mShadowDistance * sin(mShadowAngle / 180.0f * Math.PI)).toFloat()
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (mIsShadowed) {
            ...
            // Draw shadow bitmap
            if (mBitmap != null && !mBitmap!!.isRecycled) canvas.drawBitmap(
                mBitmap!!,
                0.0f,
                0.0f,
                mPaint
    ...
    companion object {
        ...
        private const val DEFAULT_SHADOW_COLOR: Int = Color.DKGRAY
